...
void ChasecamRestart (edict_t *ent)        {
         ...
         * the function. (return;) */           if (ent->owner->health <= 0)
         ...
         * old camera, but I'm lazy :) */           ChasecamStart (ent->owner);
           G_FreeEdict (ent);                }
...
/* The "ent" is the chasecam */           void ChasecamTrack (edict_t *ent)
        {                /* Create tempory vectors and trace variables */
           trace_t      tr;           vec3_t       spot1, spot2, dir;
           vec3_t       forward, right, up;           int          distance;
           int          tot;        
           ent->nextthink = level.time + 0.100;
        ...
        /* if we hit something, copy the trace end to spot2 and lower spot2 */
           if (tr.fraction < 1.000)           {
                VectorCopy (tr.endpos, spot2);                spot2[2] -= 32;
           }
        ...
        /* another traceline */
            ...
            * the player or not */
              VectorSubtract (ent->owner->s.origin, ent->s.origin, spot1);
                 ...
                 ent->velocity[0] *= 2;                  ent->velocity[1] *= 2; 
                 ent->velocity[2] *= 2;                       }        
           }
         ...
         * the new camera entity */           if (ent->chasedist2 > 3)
