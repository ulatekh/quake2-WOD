...
{
	if (coop->value && stricmp(level.mapname, "security") == 0)
	{
...
void ClientObituary (edict_t *self, edict_t *inflictor, edict_t *attacker)
{
	...
	if (deathmatch->value || coop->value)
	{
		...
		case MOD_TARGET_LASER:
			message = "tripped over the laser";
			break;
		...
		if (attacker == self)
		{
			...
			case MOD_R_SPLASH:
				if (IsFemale(self))
					message = "burnt herself up";
				else
					message = "burnt himself up";
				break;
			case MOD_H_SPLASH:
				if (IsFemale(self))
					message = "blew herself up";
			...
			case MOD_MACHINEGUN:
				if (IsFemale(self))
					message = "ate her own machine rockets";
				else
					message = "ate his own machine rockets";
				break;
			case MOD_PROXIMITY:
				if (IsFemale(self))
					message = "detonated herself";
				else
			...
			case MOD_PLASMAG:
				if (IsFemale(self))
					...
					message = "blew his face off with the plasma grenade";
				break;
			case MOD_CLUSTER:
				if (IsFemale(self))
					message = "bombed herself with grenades";
				else
					message = "bombed himself with grenades";
				break;
			case MOD_NAPALM:
				if (IsFemale(self))
					message = "napalmed herself";
				else
			...
			case MOD_CATA:
				if (IsFemale(self))
				message = "nuked herself";
				else
				message = "nuked himself";
				break;
			default:
				if (IsFemale(self))
...
qboolean ClientConnect (edict_t *ent, char *userinfo)
{
	...
	gotAddr = StringToFilter (value, &ipAddr);
